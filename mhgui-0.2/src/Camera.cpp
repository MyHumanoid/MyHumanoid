...
/*! Set internal width and height values. In Makehuman is called by Glut callbacks when it is resized.*/
...
/*! Modify orientation values. In Makehuman is called by Glut callbacks when a key is pressed.*/
...
/*! Modify orientation values. In Makehuman is called via Glut callbacks when mouse is dragged
 with a button down other than the right button..*/
...
/*! Update last mouse position. These values are used by rotateMouse() to keep track of the movement.
In Makehuman is called via Glut callbacks during a mouse drag action with a button down
other than the right button. */
...
/*! Move the camera along the xy plane. In Makehuman is called via Glut callbacks when mouse is dragged with
...
/*! Move the camera. In Makehuman is called via Glut callbacks when mouse wheel is used
...
void Camera::moveCameraAnimated(const std::string& filename, AutozoomData data,const VertexVector &vertexvector)
{
...
void Autozoom::fromStream (std::ifstream &in_stream,const std::string& filename)
{
	...
	if (sscanf (buffer, "%f,%f,%f,%f,%f,%f,%f,%f,%s", &p0, &p1, &p2, &r1, &r2, &c0, &c1, &c2, verts) == 9)
	{
...
/*! Save the current camera position to the .camera file of the active target panel. */
...
/*! Format the data coming from the current view and the vert list from the file itself. A file must already exist */
void Autozoom::createStream (std::ostringstream &out_stream,const std::string& filename,const Camera &camera)
{
	...
	out_stream 	<< camVector.x  << "," << camVector.y  << "," << camVector.z
			<< "," << camAngleX  << "," << camAngleY
			<< "," << (*this)[filename].vertsCenteroid.x  << "," << (*this)[filename].vertsCenteroid.y << "," << (*this)[filename].vertsCenteroid.z;
	std::vector<int>::iterator itVectorData;
	for (itVectorData = (*this)[filename].vertsIndexes.begin();
		    itVectorData != (*this)[filename].vertsIndexes.end();
		    itVectorData++)
	{
