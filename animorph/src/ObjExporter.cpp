	...
	for (FaceGroup::const_iterator facegroup_it = facegroup.begin();
		...
		for (unsigned int i = 0; i < facegroupdata.size(); i++) {
			...
			for (unsigned int j = 0; j < face.getSize(); j++) {
				...
				// does implicitly insert an entry for a particular has if it does not
				// exists!
...
void ObjExporter::createFullOBJStream(ostringstream &out_stream,
                                      const string &basename)
{
...
void ObjExporter::createMTLStream(std::ostringstream &out_stream,
                                  const std::string &basename)
{
	...
	for (unsigned int i = 0; i < materialvector.size(); i++) {
		...
		out_stream << "Kd " << colRGB.red() << " " << colRGB.green() << " "
		           << colRGB.blue() << endl
		           << endl;
	}
}
