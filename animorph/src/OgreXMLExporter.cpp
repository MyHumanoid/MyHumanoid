...
bool OgreXMLExporter::exportFile(const string &filename)
{
	...
	return (eXMLErrorNone ==
	        xMainNode.writeToFile((file_ending_cut + ".mesh.xml").c_str()));
}
void OgreXMLExporter::InsertASubmeshNode(XMLNode *xNodeSubmeshes, XMLNode *Node,
                                         int index_material)
{
	MaterialVector &materialvector(mesh.getMaterialVectorRef());
	TextureVector &texturevector(mesh.getTextureVectorRef());
	...
	XMLNode xNode_faces;
	XMLNode xNode_face, xNode_geometry, xNode_vertexbuffer, xNode_vertexbuffer_uv,
	    xNode_vertex;
	;
	unsigned int i, j;
	...
	xNodeSubmesh.addAttribute("material",
	                          materialvector[index_material].getName().c_str());
	...
	for (i = 0; i < facevector.size(); i++) {
		...
		if (materialvector[face.getMaterialIndex()].getName() !=
		    materialvector[index_material].getName())
			continue;
