...
PosesBsPanel::PosesBsPanel(const string &category, const Rect &rect)
...
PagePosesBsPanel *PosesBsPanel::newPagePanel()
{
	PagePosesBsPanel *page;

	page = new PagePosesBsPanel(category, getRect());

	return page;
}

void PosesBsPanel::createWidgets()
{
	...
	PagePosesBsPanel *page = newPagePanel();
	...
	for (CharactersMap::const_iterator charactersmap_it = charactersmap.begin();
		...
		if (loc == string::npos)
			...
			if (sub == category) {
				...
				image_button =
				    new ImageButton(kComponentID_TargetPanel_Target,
				                    searchPixmapFile("bsimg/" + character_image),
				                    character_name, Rect(0, 0, 64, 64));

				image_button->setListener(&imgButtonListener);
				image_button->setTooltip(
				    Tooltip(character_name, kTooltipPos, c, tooltipPanel));
				image_button->setOverlayTexture(
				    searchPixmapFile("bsimg/" + character_image));
...
/*
void PosesBsPanel::resetTargetValues(const string currentTargetName)
{
...
targetVector.begin (); targetvector_it != targetVector.end ();
  ...
  for (vector<Panel*>::iterator pagepanel_it = begin();
        pagepanel_it != end();
        pagepanel_it ++){

     ((PagePosesBsPanel*)(*pagepanel_it))->resetTargetValues(currentTargetName);
  }
}
*/
