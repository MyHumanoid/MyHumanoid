...
void PosesBsCategoryPanel::createWidgets()
{
	...
	for (int i = 0; i < 17; i++) {
		...
		cat = new Image(poses_bs_cat_table_id[i],
		                searchPixmapFile(out.str().c_str()), Rect(0, 0, 64, 64));
		cat->setListener(&selectionListener);
		cat->setTooltip(
		    Tooltip(out_tool.str().c_str(), kTooltipPos, color_red, tooltipPanel));
		cat->setAlpha(alpha);
	...
	cat = new Image(kComponentID_BsCategoryPanel_MyBS,
	                searchPixmapFile("ui/myposes.png"), Rect(0, 0, 64, 64));
	cat->setListener(&selectionListener);
	cat->setTooltip(
	    Tooltip("My Poses Library", kTooltipPos, color_red, tooltipPanel));
	cat->setAlpha(alpha);
	imageVector.push_back(cat);
	addWidget(cat);
}
