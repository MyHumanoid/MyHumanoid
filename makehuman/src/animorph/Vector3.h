...
template <typename T> class Vector3decimal : public Vector3<T>
{
public:
	Vector3decimal()
	    : Vector3<T>()
	...
	Vector3decimal(const Vector3<T> &v)
	    : Vector3<T>(v)
	{
	}
...
template <typename T> class Vector3real : public Vector3<T>
{
public:
	Vector3real()
	    : Vector3<T>()
...
public:
	...
	Vector3f(float _x, float _y, float _z)
	    : Vector3real<float>(_x, _y, _z)
	{
	}
	Vector3f(const Vector3<float> &v)
	    : Vector3real<float>(v)
	{
	}
	...
	float getMagnitude() const { return sqrtf(x * x + y * y + z * z); }
};
...
public:
	...
	Vector3d(const Vector3<double> &v)
	    : Vector3real<double>(v)
	{
	}
...
template <typename T> void Vector3real<T>::normalize()
{
...
template <typename T> T Vector3<T>::operator*(const Vector3<T> &v)
{
...
template <typename T> Vector3<T> &Vector3<T>::operator=(const Vector3<T> &v1)
{
	x = v1.x;
	y = v1.y;
	z = v1.z;
...
template <typename T> inline Vector3<T> &Vector3<T>::operator*=(const T &scalar)
{
	x *= scalar;
	y *= scalar;
	z *= scalar;
