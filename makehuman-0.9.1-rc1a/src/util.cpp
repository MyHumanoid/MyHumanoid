...
const static string aqsis_compiler_opt    (" -swrap=periodic -twrap=periodic -filter=gaussian -swidth=1 -twidth=1");
...
#endif
//paths
const static string render_templates_path         (searchDataDir("rib_data") + PATH_SEPARATOR + "rib_templates");
const static string render_templates_preview_path (searchDataDir("rib_data") + PATH_SEPARATOR + "rib_templates_preview");
const static string render_templates_toon_path    (searchDataDir("rib_data") + PATH_SEPARATOR + "rib_templates_toon");
...
{"texpath",      "${TEXTURES_PATH}", ParametersString_def[TEXTPATH_PARAM_STRING]},
...
{"eyesalpha",    "${EYES_ALPHA}",ParametersString_def[EYESALPHA_PARAM_STRING]},
{"eyebalpha",    "${EYEB_ALPHA}",ParametersString_def[EYEBALPHA_PARAM_STRING]},
{"headalpha",    "${HEAD_ALPHA}",ParametersString_def[HEADALPHA_PARAM_STRING]},
{"bodyalpha",    "${BODY_ALPHA}",ParametersString_def[BODYALPHA_PARAM_STRING]},
{"eyelalpha",    "${EYEL_ALPHA}",ParametersString_def[EYELALPHA_PARAM_STRING]},
...
{
  return g_aqsis_base_path;
}
const string GetAqsisBinPath()
{
  return (GetAqsisBasePath() + aqsis_bin);
}
const string GetAqsisRender()
{
  return (GetAqsisBinPath() + aqsis_render);
}
const string GetAqsisCompiler()
{
  return GetAqsisBinPath() + aqsis_compiler;
}
...
const string GetAqsisVersionOpt()
{
  return aqsis_version_opt;
}
const string GetAqsisCompilerOpt()
{
  return aqsis_compiler_opt;
}
const string GetAqsisShaderOpt()
{
  return aqsis_shader_compiler_opt;
}
///pixie
string g_pixie_base_path = pixie_base_path;
const string GetDefaultPixiePath()
{
  return pixie_base_path;
}
...
{
  return g_pixie_base_path;
}
const string GetPixieBinPath()
{
  return (GetPixieBasePath() + pixie_bin);
}
const string GetPixieRender()
{
  return (GetPixieBinPath() + pixie_render);
}
const string GetPixieCompiler()
{
  return GetPixieBinPath() + pixie_compiler;
}
...
const string GetPixieVersionOpt()
{
  return pixie_version_opt;
}
const string GetPixieCompilerOpt()
{
  return pixie_compiler_opt;
}
const string GetPixieShaderOpt()
{
  return pixie_shader_compiler_opt;
}
...
const StringVector getBackgroundsAlternatives (const string& data)
{
  ...
  name_vector.push_back (string (PACKAGE_BACKGROUNDS_DIR) + PATH_SEPARATOR + data);
#endif
#endif
  return name_vector;
}
...
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "myobjs" + PATH_SEPARATOR);
}

const string getMyColladaPath()
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "mycollada" + PATH_SEPARATOR);
}

const string getMyPosesPath()
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "myposes" + PATH_SEPARATOR );
}
...
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "mybs" + PATH_SEPARATOR );
}
...
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "rendering" + PATH_SEPARATOR);
}

const string getTexturesPath()
{
  //return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "textures" + PATH_SEPARATOR);
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "rendering" + PATH_SEPARATOR);
}

const string getShadersPath()
{
  //return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "shaders" + PATH_SEPARATOR);
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "rendering" + PATH_SEPARATOR);
}

const string getMyBackgroundsPath()
{
  return string(getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "mybackgrounds" + PATH_SEPARATOR);
}
...
bool compileShaders(const string& shader_compiler,const string& options , Window &mainWindow)
{
  vector<string> shaders;

  DirectoryList dir_list;
  string shaders_root_path (searchDataDir("rib_data") + PATH_SEPARATOR + "shaders_data" + PATH_SEPARATOR);
  dir_list.setRootPath (shaders_root_path);
  dir_list.setRecursive (0);
  dir_list.setFileFilter (".sl");

  string compiled_extension = (shader_compiler == GetPixieShaderCompiler() ? ".sdr" : ".slx");
         ...
         s_it++)
  {
    string cmd (" \"" + getShadersPath() + (*s_it) + compiled_extension +
                  "\" \"" + searchDataDir("rib_data") + PATH_SEPARATOR + "shaders_data" +
                  PATH_SEPARATOR + (*s_it) + ".sl\"");
...
bool compileTextures(const string& textures_dir, const string& texture_compiler,const string& options ,Window &mainWindow)
{
  cout << "entering texture" << endl;
  vector<string> textures;

  DirectoryList dir_list;
  string texture_root_path (searchDataDir("rib_data") + PATH_SEPARATOR + "textures_data" + PATH_SEPARATOR);
         ...
         s_it++)
  {
    string cmd (" \"" + searchDataDir("rib_data") + PATH_SEPARATOR +
                      "textures_data" + PATH_SEPARATOR +
                       (*s_it) + ".tif\" \"" + getTexturesPath() /*+ textures_dir + PATH_SEPARATOR*/ + (*s_it) + ".tx\"");
...
void rendering(Window &mainWindow, const RenderType type)
{
  ...
  }else if(ExecuteCommand(NMT, GetPixieRender() , GetPixieVersionOpt()) == true  )
  {
  ...
  if(!error)
  {
    if(type == PREVIEW || compileTextures(textures_dir, texture_compiler,texture_opt, mainWindow))
    {
      if(type == PREVIEW || compileShaders(shader_compiler, shader_opt,mainWindow))
      {
...
//          StringPair strTexturesPath ("${TEXTURES_PATH}", getTexturesPath() /*+ textures_dir*/ + PATH_SEPARATOR);

          replaceList.push_back (strShadersPath);
//          replaceList.push_back (strTexturesPath);

          if(!strcasecmp(editParametersString[TEXTPATH_PARAM_STRING].value.c_str(), "default"))
            oss << (getTexturesPath() /*+ textures_dir*/ + PATH_SEPARATOR);
          else
            oss << editParametersString[TEXTPATH_PARAM_STRING].value;

          StringPair strTexturesPath   (editParametersString[TEXTPATH_PARAM_STRING].proto, oss.str());
          replaceList.push_back (strTexturesPath);
...
//----------------RIB PARAMETERS -----------------//
        //string
          ...
          if(! strcasecmp(editParametersString[RENDPATH_PARAM_STRING].value.c_str(),"default"))
            oss << getRenderingPath();
          else
            oss << editParametersString[RENDPATH_PARAM_STRING].value;

          StringPair strParam   (editParametersString[RENDPATH_PARAM_STRING].proto, oss.str());
          replaceList.push_back (strParam);


        for (int i = HEADCOLOR_PARAM_STRING; i < LAST_PARAM_STRING ;i++){

          StringPair strParam   (editParametersString[i].proto, editParametersString[i].value.c_str());
          replaceList.push_back (strParam);
        }
//------------------------------------------------//
...
void renderingStep()
{
  ...
  if(sl_it != sl_end)
  {
    ...
    ribExporter.exportFile (used_templates_path, template_name,
                            getRenderingPath() + out_file,
                            replaceList);

 //   callRender(render + " \"" + getRenderingPath() + out_file + "\"", mainWindow);
...
int ParseStringRIBParameter(char *param, char *value)
{
  ...
  if( !strcasecmp(parameter.c_str(), editParametersString[RENDPATH_PARAM_STRING].name) ){
    ...
    if(dirHd != NULL)
    {
      // Yes, then release the handle again and return
      ::closedir(dirHd);
       editParametersString[RENDPATH_PARAM_STRING].value = str_value + PATH_SEPARATOR;
       return 0;
    }
  ...
  if( !strcasecmp(parameter.c_str() , editParametersString[TEXTPATH_PARAM_STRING].name) ){
    ...
    if(dirHd != NULL)
    {
      // Yes, then release the handle again and return
      ::closedir(dirHd);
       editParametersString[TEXTPATH_PARAM_STRING].value = str_value + PATH_SEPARATOR;
       return 0;
    }
    return -3;//path not found!
  }
  ...
  for (unsigned int j = 0;j<size_vx;j++){
    ...
    if(symm_vertex[j] == -1){  //sx vertex
      ...
      sscanf((char*)out_stream[symm].str().c_str(),
             ...
             &index, &weight[0],  &index, &weight[1],  &index, &weight[2],  &index, &weight[3],  &index, &weight[4],  &index, &weight[5],  &index, &weight[6],  &index, &weight[7],  &index, &weight[8],  &index, &weight[9],
             &index, &weight[10], &index, &weight[11], &index, &weight[12], &index, &weight[13], &index, &weight[14], &index, &weight[15], &index, &weight[16], &index, &weight[17], &index, &weight[18], &index, &weight[19],
             &index, &weight[20], &index, &weight[21], &index, &weight[22], &index, &weight[23], &index, &weight[24], &index, &weight[25], &index, &weight[26], &index, &weight[27], &index, &weight[28], &index, &weight[29],
             &index, &weight[30], &index, &weight[31], &index, &weight[32], &index, &weight[33], &index, &weight[34], &index, &weight[35], &index, &weight[36], &index, &weight[37], &index, &weight[38], &index, &weight[39],
             &index, &weight[40], &index, &weight[41], &index, &weight[42], &index, &weight[43], &index, &weight[44], &index, &weight[45], &index, &weight[46], &index, &weight[47], &index, &weight[48], &index, &weight[49],
             &index, &weight[50], &index, &weight[51], &index, &weight[52], &index, &weight[53], &index, &weight[54], &index, &weight[55], &index, &weight[56], &index, &weight[57], &index, &weight[58], &index, &weight[59],
             &index, &weight[60], &index, &weight[61], &index, &weight[62], &index, &weight[63], &index, &weight[64], &index, &weight[65], &index, &weight[66], &index, &weight[67]);
...
void ExportConfigurationXML()
{
  ...
  XMLNode xNode_rendering_parameters,xNode_collada_parameters,xNode_aqsis_path, xNode_pixie_path, xNode_rendering_path, xNode_texture_path;

  XMLNode xNode_head_texture,xNode_body_texture,xNode_head_bump,xNode_body_bump;

  XMLNode xNode_km, xNode_Veins, xNode_Wrinkles, xNode_oilVal, xNode_sssScale, xNode_Ka, xNode_ColorR,xNode_ColorG,xNode_ColorB;
  XMLNode xNode_DisplayX, xNode_DisplayY;

  XMLNode xNode_eyescol,xNode_eyelcol,xNode_eyebcol,xNode_headspec, xNode_bodyspec;
  ...
  xNode_Wrinkles.addText(editParametersString[WRINKLES_PARAM_STRING].value.c_str());
  ...
  xNode_sssScale.addText(editParametersString[SSSSCALE_PARAM_STRING].value.c_str());
  ...
  xNode_DisplayX.addText(editParametersString[DISPLAYX_PARAM_STRING].value.c_str());

  //DisplayY
  xNode_DisplayY = xNode_rendering_parameters.addChild("DisplayY");
  xNode_DisplayY.addAttribute("command","set DisplayY");
  xNode_DisplayY.addText(editParametersString[DISPLAYY_PARAM_STRING].value.c_str());
  ...
  XMLError error = xMainNode.writeToFile((getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "makehuman.parameters").c_str());
...
void ParseConfigurationXML()
{
  ...
  string file = (getUserWorkDir() + "makehuman" + PATH_SEPARATOR + "makehuman.parameters");
  ...
  xNode_rendering_parameters = xNode_makehuman.getChildNode("rendering_parameters");
  ...
  xNode_rendering_path = xNode_rendering_parameters.getChildNode("rendering_path");
  ...
  xNode_texture_path = xNode_rendering_parameters.getChildNode("texture_path");
  ...
  xNode_head_texture = xNode_rendering_parameters.getChildNode("head_color");
  ...
  xNode_body_texture = xNode_rendering_parameters.getChildNode("body_color");
  ...
  XMLNode xNode_eyescol,xNode_eyebcol,xNode_eyelcol,xNode_headspec, xNode_bodyspec;
  ...
  xNode_eyelcol = xNode_rendering_parameters.getChildNode("eyelashes_color");
  ...
  xNode_eyebalpha = xNode_rendering_parameters.getChildNode("eyebrows_alpha");
  ...
  xNode_eyelalpha = xNode_rendering_parameters.getChildNode("eyelashes_alpha");
...
//--------------------------//
  ...
  xNode_collada_parameters = xNode_makehuman.getChildNode("collada_parameters");
  if(!xNode_collada_parameters.isEmpty()){
  xNode_exp_controller = xNode_collada_parameters.getChildNode("export_controller");
...
bool loadSelectorsPositions(const std::vector<string>& strings, const float value)
{
      ...
      it++)
  {
    ...
    {
      global.setFuzzyValue(id, Point((int32_t)(x * value), (int32_t)(y * value)));
    }
  }
...
bool saveSelectorsPositions(const std::string& filename, std::ios_base::openmode mode)
{
  ...
  if(tmp != NULL)
  {
    out_stream << "#t,A," << kComponentID_CharacterSettingPanel_Age << "," << tmp->getX() << "," << tmp->getY() << endl;
  }

  tmp = global.getFuzzyValue(kComponentID_CharacterSettingPanel_MuscleSize);
  if(tmp != NULL)
  {
    out_stream << "#t,M," << kComponentID_CharacterSettingPanel_MuscleSize << "," << tmp->getX() << "," << tmp->getY() << endl;
  }

  tmp = global.getFuzzyValue(kComponentID_CharacterSettingPanel_Breast);
  if(tmp != NULL)
  {
    out_stream << "#t,B," << kComponentID_CharacterSettingPanel_Breast << "," << tmp->getX() << "," << tmp->getY() << endl;
  }

  tmp = global.getFuzzyValue(kComponentID_CharacterSettingPanel_Shape);
  if(tmp != NULL)
  {
    out_stream << "#t,S," << kComponentID_CharacterSettingPanel_Shape << "," << tmp->getX() << "," << tmp->getY();
  }
